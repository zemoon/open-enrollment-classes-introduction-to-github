...
# Get list of new topojson files
geojsonfiles=$(find . -name '*.topojson' -not -name 'class_pins.topojson')

# If there are no files to build/deploy, we exit the build
if [ -z "$geojsonfiles" ]; then
  echo "No new files found. Aborting deployment..."
  exit 0
else
  echo "topojson files found:"
  echo $geojsonfiles
fi
...
git add -A
git commit -m "Automatic commmit: merge topojson files" # we don't want to trigger a build -- the reason we're doing this to begin with is because we're in the middle of a build
